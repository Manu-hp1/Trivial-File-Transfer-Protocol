...
/* FIX: Unified client structure with server_ fields
   Used by both server (to track connected client) and client (to track server) */
typedef struct
{
    int sockfd;                     // Socket file descriptor
    struct sockaddr_in server_addr; // Server address (used by client side)
    socklen_t server_len;           // Server address length
    char server_ip[27];             // Server IP address
